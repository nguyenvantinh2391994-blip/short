...
class ChromeTokenExtractor:
    ...
    def call_api_with_captured_payload(
        ...
        try:
            ...
            # Xử lý 429 - Rate limit / Quota exhausted (không retry ở đây, để main_tab retry với token mới)
            if response.status_code == 429:
                if callback:
                    callback(f"⚠️ Quota limit (429) - cần retry với token mới")
                return []
            ...
            if response.status_code == 403:
                ...
                if "recaptcha" in resp_text:
                    if callback:
                        callback("❌ recaptchaToken đã hết hạn - cần retry với token mới")
                else:
                    if callback:
            ...
            if callback:
                callback("❌ Request timeout - cần retry")
            return []
        except Exception as e:
            if callback:
                callback(f"❌ Error: {e}")
